...
Gemini와 Claude의 기본값(예: `gemini-3.7-flash`, `claude-opus-5`)은 각 스킬의 `scripts/workflow/models.py` 한 곳에 정의됩니다. Codex는 계정에서 지원하지 않는 ID를 고정하지 않도록 인증된 CLI가 지원하는 모델을 고르는 `codex-default`를 기본값으로 사용합니다. 지원이 종료되거나 세대가 지난 구체적 모델 ID는 실행 전에 거부합니다. 멀티 모델 오케스트레이션은 하나의 설계상 선택일 뿐이며 **더 나은 결과를 보장하지 않습니다.** 모델 출력은 신뢰할 수 없는 것으로 간주하여 검토하고, 제3자 콘텐츠를 입력할 때는 프롬프트 인젝션(prompt-injection)에 유의하세요.

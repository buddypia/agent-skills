...
description: |
  Route and audit how Claude Code is steered across its 7 mechanisms (CLAUDE.md / rules / skills / subagents / hooks / output styles / append-system-prompt), choosing each instruction's home by 4 axes — load timing, compaction survival, context cost, authority. Use whenever — (1) ROUTE: "where should this instruction live?" or any request to make a behavior permanent; (2) AUDIT: scan and refactor an existing project's .claude config against the official docs; (3) symptoms of misplacement — "CLAUDE.md has bloated", "Claude ignores a rule", "make X run every time", "never let Claude do X", "context feels heavy". Trigger even without the word "steering" — "where should this go", "clean up my CLAUDE.md", "audit my .claude", どこに書く / CLAUDE.md を整理.
---

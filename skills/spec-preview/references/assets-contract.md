...
| `preview.js` | viewport / mode / embed / iframe fit / チェックリスト制御 | **無編集で**コピー |
...
| index | `[data-checklist]` / `li[data-check-item]` / `.check-actions` / `[data-checklist-progress]` | レビューチェックリスト（OK/要修正トグル + localStorage 保存 + 進捗表示）。マークアップ契約の正本は `references/review-checklist.md`。ボタンラベル・進捗文言は `html lang` から JS が自動選択（`__LABEL_*__` ではない） |

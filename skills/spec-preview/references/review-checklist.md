...
判定（OK / 要修正）が localStorage に保存され、進捗が集計表示される。
...
2. **下の項目プールから該当セルの項目だけ**選ぶ（●=必須 / ○=該当時のみ / −=出さない）:
   ...
   | **状態網羅** | loading / empty / error / 长文・データ 0 件時の表示が壊れない | ● | ○ | ○ | − | − |
...
- 判定は `li[data-state="ok"|"ng"]` として付与され、localStorage
  （`spec-preview-check:<pathname>:<id>`）に保存 → リロードしても保持。
- ボタンラベル・進捗文言は `html lang`（ja/ko/en）から preview.js が自動選択する
...
- 「チェックリストを確認して進めてよいですか？」
  選択肢 = 「全項目 OK、進めて」 / 「指摘あり（内容をチャットで伝える）」 / 「保留」
- ユーザーが「指摘あり」の場合、どの項目（slug）かを聞き、修正タスクにそのまま渡す。
